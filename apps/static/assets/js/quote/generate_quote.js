$(function() {
    ...
    const subscription = dt_to.subscribe(Namespace.events.change, (e) => {
        dt_from.updateOptions({
          restrictions: {
            maxDate: e.date,
          },
        });
    ...
    $('#dtt, #dtf').on('change', () => {
        if ($('#dtt').val() != '' && $('#dtf').val() != '') {
            ...
            finish_bnt.off('click').on('click', () => {
                ...
                // var view_type = document.createElement('input') 
                ...
                // for (const v of [match_ids, order_by, date_range_from, date_range_to, view_type]) {
                //     form.appendChild(v)
                // }
